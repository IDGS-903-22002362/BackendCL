# Loyalty — Deuda de tests fuera del pipeline crítico

Suites fallidas preexistentes (inventario, ofertas, etc.) documentadas para seguimiento. No usar `skip`.

Pipeline crítico obligatorio:

- `npm run build`
- `npm run validate:openapi`
- `npm run test:critical:loyalty`

Suites conocidas fuera del scope loyalty (jun 2026): revisar con `npm test` completo y clasificar en PRs separados.
