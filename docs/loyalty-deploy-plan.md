# Plan de despliegue progresivo — Loyalty v1

1. Backup producción
2. Desplegar índices Firestore
3. Desplegar functions con flags OFF (`configuracion/loyalty`)
4. Health check + logs
5. Migración dry-run → revisión → migración real → conciliación
6. Activar flags: reads → physical earn → bonus → digital → reversals → redemptions
7. Mantener adaptadores legacy ON; monitorear `legacy_loyalty_endpoint_used`
8. Sunset legacy solo con tráfico cero documentado
