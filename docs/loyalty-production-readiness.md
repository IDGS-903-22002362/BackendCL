# Loyalty v1 — Preparación para producción

**Veredicto:** LISTO PARA PRODUCCIÓN (condicionado — validación app móvil pendiente)

**Despliegue:** 2026-07-01 | Projects: `e-comerce-leon` (Functions) + `app-oficial-leon` (Firestore puntos)

## Evidencia de despliegue

| Item | Resultado |
|------|-----------|
| Backup PITR + export | `gs://app-oficial-leon.firebasestorage.app/backups/loyalty-pre-deploy-20260701-110138` |
| Índices loyalty | Desplegados en app-oficial-leon/(default) |
| Functions + crons | Desplegados en e-comerce-leon |
| Frontend App Hosting | Desplegado |
| Migración | 29 usuarios, 44 tx, idempotente en 2da pasada |
| Conciliación | 0 críticas, 1 advertencia legacy_history_incomplete |
| Flags finales | Fase E, `legacyPointsAdaptersEnabled=true` |
| Tests críticos | 8 suites, 30 tests OK |

Ver scripts: `npm run migrate:ledger`, `reconcile:ledger`, `activate:loyalty-flags`.

---

**Veredicto anterior (certificación interna):** NO LISTO PARA PRODUCCIÓN (certificación interna en progreso)

## Cambios implementados

- Idempotencia y `externalTransactionId` dentro de transacción Firestore
- Wallet como fuente de verdad cuando existe (sin sync inverso desde `puntosActuales`)
- Expiración vía `loyaltyEngineService.processExpirationsVencidas`
- Feature flags en `configuracion/loyalty` + env overrides
- Proxy frontend corregido (`/api/loyalty/*` → `/api/loyalty/v1/*`)
- Tests: legacy adapters, concurrencia/idempotencia, conversión, welcome bonus
- Índices Firestore adicionales para admin (`actorId`, `channel`, `externalTransactionId`)

## Pipeline crítico

```bash
cd functions && npm run build && npm run validate:openapi && npm run test:critical:loyalty
cd ../TiendaFrontCL && npm run build
```

## Pendiente antes de producción

- Proyecto Firebase staging independiente + migración real + conciliación
- Pruebas E2E con app móvil contra adaptadores legacy
- Verificación post-deploy de crons y Cloud Scheduler
