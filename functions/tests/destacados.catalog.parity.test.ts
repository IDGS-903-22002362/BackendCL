/**
 * Home Destacados rail and /products?sort=destacados must share the same
 * backend ranking (getDestacadosRankedProductIds + listCatalogProductsByDestacados).
 */
...
describe("Destacados catalog parity (home + /products?sort=destacados)", () => {
  beforeEach(() => {
    dbState = {
      categorias: {
  ...
  it("returns analytics-ranked IDs in order with onlyAvailable=true (default storefront query)", async () => {
  ...
  it("paginates destacados without reordering (home slice vs catalog first page)", async () => {
